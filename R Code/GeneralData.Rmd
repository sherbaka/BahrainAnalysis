...
KuwaitWeather<- KuwaitWeather %>%
select(Year, Month, Day, Hour, tempc, relh, wd, ws, visibility)  %>%
        mutate(date = make_datetime(Year, Month, Day, Hour))
...
```{r}
write.csv(KuwaitFullJoin, file= "KuwaitFullJoin.csv")
...
setwd("~/Desktop/4Years@Reed_2/Summer Research/BahrainAnalysis")
ManamaWeather <- read_csv("Data/ManamaWeather.csv")
names(ManamaWeather)[names(ManamaWeather)=="tmpf"] <- "tempf"
...
ManamaWeather<- ManamaWeather %>%
dplyr::select(Year, Month, Day, Hour, visibility, wd, ws, relh, tempf) %>%
   mutate( date = make_datetime(Year, Month, Day, Hour )  )
is.na(ManamaWeather$wd) <- startsWith(ManamaWeather$wd, "M")
ManamaWeather$wd <- as.numeric(ManamaWeather$wd)
ManamaWeather$relh <- as.numeric(as.character(ManamaWeather$relh))
ManamaWeather$tempf <- as.numeric(as.character(ManamaWeather$tempf))
ManamaWeather <- ManamaWeather %>%
  mutate(
    YearName = case_when(
      Year =='2016' ~ "2016",
...
```{r}
write.csv(ManamaFullJoin, file= "ManamaFullJoin.csv")
write.csv(ManamaWeather, file= "ManamaWeather.csv")
write.csv(ManamaPMFull, file= "ManamaPMFull.csv")
